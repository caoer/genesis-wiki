---
description: The seed tree — laid out exactly as it lands relative to a child wiki's root (contract `C49-upstream-layout`). Birth = verbatim copy + placeholder fill.
...
- Multi-line slots — `{{REFERENCE_WIKIS_BLOCK}}` (YAML array or `[]`) and `{{IDENTITY_PARAGRAPH}}` — are spliced from birth parameters (the INSTALL.md `^bootstrap` params-file).
...
- No `lefthook.yml` — the INSTALL.md `^install` run writes the role-selected hook (private = none). Contract `C45-role-selects-lint-pack`.
